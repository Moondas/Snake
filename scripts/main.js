"use strict";
var can = document.getElementById("myCan"), c = can.getContext('2d');
/* Canvas dimensions */
var cw = can.width, ch = can.height, Step = { ver: 0, hor: 0 };
/* Base functions */
...
var hud = {
    ...
    show: function (elm) {
        if (Array.isArray(elm)) {
            ...
            this.getElm(elm).style.display = "initial";
    },
    hide: function (elm) {
        if (Array.isArray(elm)) {
            for (var i = 0; i < elm.length; i++) {
            ...
            this.getElm(elm).style.display = "none";
    },
};
/* Snake */
var snake = {
    ...
    move: function (direction) {
        ...
        switch (dir) {
            /* Y axis */
            case "up":
                coords.unshift({ x: head.x, y: head.y - 1 });
                break; // Y-
            case "down":
                coords.unshift({ x: head.x, y: head.y + 1 });
                break; // Y+
            /* X axis */
            case "left":
                coords.unshift({ x: head.x - 1, y: head.y });
                break; // X-
            case "right":
                coords.unshift({ x: head.x + 1, y: head.y });
                break; // X+
    ...
    draw: function () {
        ...
        c.fillStyle = "#030";
        // Draw Snake's head
        c.fillRect(head.x * grid.getSize() + 1, head.y * grid.getSize() + 1, grid.getSize() - 2, grid.getSize() - 2);
        // Hide Snake's track
        if (!this.isGrow) {
            c.fillStyle = "darkgreen";
            c.fillRect(this.afterCoord.x * grid.getSize() + 1, this.afterCoord.y * grid.getSize() + 1, grid.getSize() - 2, grid.getSize() - 2);
        }
        c.stroke();
    }
};
...
var food = {
    ...
    place: function () {
        var coords = this.genRandCoord(), pi = Math.PI, r = grid.getSize() / 2;
